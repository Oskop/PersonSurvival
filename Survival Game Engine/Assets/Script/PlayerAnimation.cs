...
public class PlayerAnimation : MonoBehaviour {
	...
	void Update () {
		...
		bool notRunningForwardOrStanding = v >= 0;
		...
		if ( (notRunningForwardOrStanding && runningRight) || (runningBackward && runningLeft) ) {
			_animation.Blend ("RunRight");
			_animation.Blend ("Idle", 0);
			_animation.Blend ("RunLeft", 0);
		} else if ( (notRunningForwardOrStanding && runningLeft) || (runningBackward && runningRight) ) {
